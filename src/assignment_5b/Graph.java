 ...
 *
 *Completing the implementation of Graph.java
 */
...
public class Graph<T> implements GraphADT<T> {
    ...
    public void removeEdge(int index1, int index2) {
        ...
        if (indexIsValid(index1) && indexIsValid(index2)) {//if valid indexes in graph
            adjMatrix[index1][index2] = false;//position set to false to remove the edge
            adjMatrix[index2][index1] = false;//position set to false to remove the edge
            modCount++;
        } else {//if invalid indexes throw exception
    ...
    public void addVertex(T vertex) {
        // To be completed as a Programming Project
        if ((numVertices + 1) == adjMatrix.length) {//expand array if full
            expandCapacity();
        }

        vertices[numVertices] = vertex;//new vertex set to null
        for (int i = 0; i < numVertices; i++) {//loop through to set vertices to false
    ...
    public void removeVertex(int index) {
        ...
        if (indexIsValid(index)) {
            ...
            for (int i = index; i < numVertices; i++) {
                vertices[i] = vertices[i + 1];//shifting vertices
            }
            for (int i = index; i < numVertices; i++) {
                for (int j = 0; j <= numVertices; j++) {
                    adjMatrix[i][j] = adjMatrix[i + 1][j];//shift up
                }
            }
            for (int i = index; i < numVertices; i++) {
                for (int j = 0; j < numVertices; j++) {
                    adjMatrix[j][i] = adjMatrix[j][i + 1];//shilf left
                }
            }
            modCount++;
    ...
    /**
     * Returns an iterator that performs a breadth first traversal starting at
     * the given index.
     *
     * @param startIndex the index from which to begin the traversal
     ...
     * @return the an iterator containing the indices of the of the vertices
     * making the shortest path between the given indices
     */
    ...
    public boolean isConnected() {
        // To be completed as a Programming Project
        //connected if iterator returnes all vertices 
        Iterator<T> iterator = iteratorBFS(0);

        int count = 0;//vertices counted

        while (iterator.hasNext()) {
            iterator.next();//next vertex
            count++;
        }
    ...
    public int getIndex(T vertex) {
        // To be completed as a Programming Project
        int index = -1;//set to -1
        for (int i = 0; i < numVertices; i++) {
            if (Objects.equals(vertices[i], vertex)) {//if current one is target
                index = i;//index updated
                break;
            }
        }
        return index;//-1 returned if result not found
    }
    ...
    protected boolean indexIsValid(int index) {
        // To be completed as a Programming Project
        return (index >= 0) && (index <= numVertices);//bounds to determine valid index
    }
    ...
    public Object[] getVertices() {
        // To be completed as a Programming Project
        Object[] result = new Object[numVertices];//new object to obtain verticies

        System.arraycopy(vertices, 0, result, 0, numVertices);//create copy
        return result;
    }
...
/*
Assistance from Chapter 24: Graphs
Assistance from: 
    www.opendatastructures.org/ods-java/12_1_AdjacencyMatrix_Repres.html
Assistance from: 
    www.stackoverflow.com/questions/6010096/good-way-to-delete-vertices-from-adjacency-matrix
*/
